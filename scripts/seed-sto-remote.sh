#!/bin/bash
# STO 원격(AWS) DynamoDB 시드 — scripts/sto-seed-data.json 을 biyard-{stage}-sto 테이블에 적재.
# 사용법:
#   STAGE=dev  AWS_REGION=ap-northeast-2 bash scripts/seed-sto-remote.sh
#   STAGE=prod AWS_REGION=ap-northeast-2 bash scripts/seed-sto-remote.sh
# 또는 Makefile 의 `make seed-sto ENV=dev` 로 호출.
...
# 테이블 존재 확인
if ! aws --region "$AWS_REGION" dynamodb describe-table --table-name "$TARGET_TABLE" >/dev/null 2>&1; then
...
for i in $(seq 0 25 $((TOTAL - 1))); do
    ...
    if aws --region "$AWS_REGION" dynamodb batch-write-item \
...
ACTUAL=$(aws --region "$AWS_REGION" dynamodb scan \
