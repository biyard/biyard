...
fn decode_string(hex: &str) -> String {
    ...
    // word 1: length
    let len = u64::from_str_radix(&hex[64..128].trim_start_matches('0'), 16).unwrap_or(0) as usize;
    if len == 0 {
        return String::new();
    }
    let data_start = 128;
...
pub async fn fetch_treasury_status(
    ...
    // 1. Get stable token address from treasury
    ...
    // 2. Query stable token: balanceOf(treasury), decimals(), symbol()
    ...
    // 3. Query brand token + treasury info
...
async fn futures_join3<A, B, C>(
    ...
    // In WASM single-threaded context, we run sequentially
    let a = a.await;
    let b = b.await;
    let c = c.await;
    (a, b, c)
