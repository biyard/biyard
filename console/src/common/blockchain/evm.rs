...
pub async fn deploy_token(
    ...
    let _ = SupportedChain::from_chain_id(chain_id).ok_or_else(|| format!("Unsupported chain: {chain_id}"))?;
    ...
    let bytecode_hex = std::env::var("BIYARD_TOKEN_BYTECODE")
        .map_err(|_| "BIYARD_TOKEN_BYTECODE env var not set".to_string())?;
...
pub fn encode_mint_calldata(to_address: &str, amount: u64) -> Result<String, String> {
    ...
    Ok(format!("0x{}", hex::encode(ethers::abi::AbiEncode::encode(calldata))))
}
