...
pub async fn get_claim_signature_handler(
    ...
    // Use on-chain currentMonth (includes advanceMonth offset)
    let provider = crate::common::blockchain::provider(chain_id)?;
    ...
    // Calculate max_claimable for this user/month
    let (bal_pk, bal_sk) =
    ...
    let total_points = agg.awarded_points.max(1);
    let user_points = balance.total_earned;
    ...
    let max_claimable: u128 = user_points as u128 * user_pool / total_points.max(1) as u128;

    let amount: u128 = amount
        .parse()
        .map_err(|_| TokenError::MintFailed("Invalid amount".to_string()))?;

    if amount > max_claimable {
        return Err(TokenError::MintFailed(format!(
            "Amount {amount} exceeds max claimable {max_claimable}"
        ))
        .into());
    }
    ...
    // Even if the on-chain claim fails later, the server has committed to
    // this signature — the user can retry with the same signature until
    ...
    let point_deduction = PointBalance::updater(bal_pk2, bal_sk2)
        ...
        .with_total_earned(0)
        .decrease_balance(amount as i64)
        .increase_total_spent(amount as i64)
        .with_updated_at(now);
    ...
    let aggregation_update = MonthlyPointAggregation::updater(agg_pk2, agg_sk2)
        .increase_exchanged_points(amount as i64)
        .with_updated_at(now);
...
fn month_str_to_index(month_str: &str, token: &ProjectToken) -> u64 {
    ...
    let created_at_ms = token.created_at;

    let target_months_since_epoch = (year - 1970) * 12 + (month_num - 1);
    let created_months_since_epoch = created_at_ms / 1000 / (30 * 24 * 3600);

    let diff = target_months_since_epoch - created_months_since_epoch;
